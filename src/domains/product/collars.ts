import { Product } from "@/models/Product";

export const collarsItems: Product[] = [
  {
    id: 301,
    imageUrls: ["/images/collars/collars-2.jpg"],
    title: "title",
    price: 10,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce a mollis leo. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Suspendisse a pulvinar arcu.",
    category: "collars",
    inStock: {
      en: "pre-order",
      uk: "під замовлення",
    },
  },
  {
    id: 302,
    imageUrls: ["/images/collars/white-collar.jpg"],
    title: "title",
    price: 10,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce a mollis leo. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Suspendisse a pulvinar arcu.",
    category: "collars",
    inStock: {
      en: "pre-order",
      uk: "під замовлення",
    },
  },
  {
    id: 303,
    imageUrls: ["/images/collars/white-collar-with-green-leaves.jpg"],
    title: "title",
    price: 10,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce a mollis leo. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Suspendisse a pulvinar arcu.",
    category: "collars",
    inStock: {
      en: "pre-order",
      uk: "під замовлення",
    },
  },
  {
    id: 304,
    imageUrls: ["/images/collars/collars-cover.jpg"],
    title: "title",
    price: 10,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce a mollis leo. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Suspendisse a pulvinar arcu.",
    category: "collars",
    inStock: {
      en: "pre-order",
      uk: "під замовлення",
    },
  },
];
